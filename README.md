...
4. Если в папке много фото одной камерой подряд — необязательное
   ...
   "Digit count" ниже)
5. Экран ручной проверки: таблица с миниатюрой, именем файла и
   распознанным номером по каждому фото — номер можно поправить или
   вписать вручную там, где он не распознался, прежде чем что-либо
   переименовывать
6. Копия файла с новым именем `<номер>_<исходное_имя>` сохраняется в
...
- **Process** — запускает распознавание; по готовности открывается окно
  проверки (миниатюра + имя файла + номер, поле редактируемое). Пустой
  номер — файл не переименовывается. Кнопка **Переименовать** в этом
  окне копирует файлы в `<папка>/renamed/` с журналом `manifest.json`;
  **Отмена** закрывает окно, ничего не трогая
